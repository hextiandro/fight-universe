-- La posición en la cartelera se publica: el planeta muestra solo la parte alta
-- (si no, 500 nodos ahogan el navegador) y el panel sigue enseñando la cartelera entera.
  ...
  'fights', (
    select coalesce(jsonb_agg(b.value order by b.date, b.id), '[]'::jsonb) from (
      select fg.date, fg.id, jsonb_strip_nulls(jsonb_build_object(
        ...
        'status', case when fg.status = 'scheduled' then 'scheduled' end,
        'cardPosition', fg.card_position,
