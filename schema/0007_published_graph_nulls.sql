-- Corrige la función: winnerId y holderId se conservan aunque sean null.
  ...
  'developments', (
    select coalesce(jsonb_agg(d.value order by d.date, d.slug), '[]'::jsonb) from (
      select dv.date, dv.slug, jsonb_strip_nulls(jsonb_build_object(
        ...
        -- holderId null significa "vacante": el campo debe existir igualmente
        'titleChange', case when dv.division_id is null then null else jsonb_build_object(
